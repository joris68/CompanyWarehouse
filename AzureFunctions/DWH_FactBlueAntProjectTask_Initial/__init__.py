...
import json
import traceback
from common import get_relative_blob_path, init_DWH_Db_connection, close_DWH_DB_connection, get_latest_blob_from_staging, insert_into_loadlogging, DWH_table_logging,get_time_in_string,insert_into_tmp_table,truncate_tmp_table,get_blob_start_with
    ...
    try:
        ...
        for item in data:
            for task in item['entries']:
                ...
                taskid=task['id']
                if "workActualMinutes" in task:
                    workActualMinutes = task["workActualMinutes"]
                else :
                    workActualMinutes = -1

                if "workActualDays" in task:
                    workActualDays = task["workActualDays"]
                else :
                    workActualDays = -1

                if "workPlannedMinutes" in task:
                    workPlannedMinutes = task["workPlannedMinutes"]
                else :
                    workPlannedMinutes = -1
                    
                if "workPlannedDays" in task:
                    workPlannedDays = task["workPlannedDays"]
                else :
                    workPlannedDays = -1

                if "workEstimatedMinutes" in task:
                    workEstimatedMinutes = task["workEstimatedMinutes"]
                else :
                    workEstimatedMinutes = -1

                if "workEstimatedDays" in task:
