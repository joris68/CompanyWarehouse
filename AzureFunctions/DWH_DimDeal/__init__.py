...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        # Convert the bytes to a file-like byte stream using io.BytesIO
        data_stream = io.BytesIO(blob_content)
        
        # Read the stream into a pandas DataFrame object, usecols is used to only select the columns that are needed. More result in an error 
        data = pd.read_csv(data_stream, sep=',', encoding='utf-8', header=None, engine='python', usecols=range(110))
        
        # Filter the data to only include the columns that are needed
        data = data.iloc[:, [3, 0, 64, 59, 14, 65, 102]]
        data.columns = ['Status','DealBK','DealName','CustomerName','DealStateID','DealPlannedDays','LastChangedDateID']
