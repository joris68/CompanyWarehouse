...
import hashlib
import json
from common import get_relative_blob_path, get_latest_blob_from_staging, truncate_tmp_table, insert_into_tmp_table, init_DWH_Db_connection, close_DWH_DB_connection, insert_into_loadlogging, DWH_table_logging,get_time_in_string,get_last_two_SprintID
import traceback
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        # -----------------------------------------------------------
        # Load the latest xml-file from the staging area "jiraissuekeys"
        # -----------------------------------------------------------
 ...
 #   '// TO DO: In the CTE Query is the Left Join on DimJiraSprint and DimJiraIssueType missing.' 
        ...
        THEN 
            UPDATE SET 
                TARGET.[HashkeyValue] = SRC.[HashkeyValue],
        ...
        cursor.execute(f"""
            ...
            BEGIN TRY 
                BEGIN 
                    -- DeleteFlag der letzten 2 Sprints
