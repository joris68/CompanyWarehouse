    ...
    #       datetime_obj = datetime.strptime(date[x][:-5], "%Y-%m-%d")
        ...
        cursor.execute(f"""
            ...
            BEGIN 
                ...
                -- 1. Tag des vorletzten Monats
                DECLARE @FirstDayOfPrevMonth DATE = DATEADD(MONTH, -2, @FirstDayOfCurrentMonth);

                DECLARE @FirstDayOfPrevMonthID INT = CAST(CONVERT(varchar(10),convert(date,@FirstDayOfPrevMonth),112) as INT);

                SELECT @FirstDayOfPrevMonthID;

                DELETE FROM [{SCHEMA_NAME}].[{TABLE_NAME}]
                WHERE WorklogDateID >= @FirstDayOfPrevMonthID;
                ...
                WITH cte AS (
                    ...
                    LEFT JOIN 
                        [{SCHEMA_NAME}].DimBlueAntProject b ON a.ProjectBK = b.ProjectBK
                    LEFT JOIN 
                        [{SCHEMA_NAME}].DimUser c ON a.BlueAntUserBK = c.BlueAntUserBK
                    LEFT JOIN 
                        [{SCHEMA_NAME}].DimBlueAntProjectTask d ON a.BlueAntTaskBK = d.BlueAntTaskBK
                )
